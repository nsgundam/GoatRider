...
export const roomHandler = (io: Server, socket: Socket) => {
  ...
  socket.on(
    "join_room",
    async (data: { roomId: string; walletAddress: string }) => {
      ...
      try {
        ...
        if (room.status !== "WAITING") {
          if (room.status === "IN_GAME" && isMember) {
            // อนุญาตให้ Re-join ได้ (ผ่านไปทำงานต่อข้างล่าง)
            console.log(`🔄 User ${walletAddress} re-joining active game...`);
          } else {
            socket.emit("error", "Game already started or finished");
            return;
          }
        } else {
          // ถ้าห้อง WAITING แต่เต็ม
